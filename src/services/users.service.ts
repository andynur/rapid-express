...
export class UserService {
  ...
  public async findUserById(userId: bigint): Promise<User> {
  ...
  public async updateUser(userId: bigint, userData: CreateUserDto): Promise<User> {
    const findUser: User = await this.user.findUnique({ where: { id: userId } });
    if (!findUser) throw new HttpException(409, "User doesn't exist");
  ...
  public async deleteUser(userId: bigint): Promise<User> {
    const findUser: User = await this.user.findUnique({ where: { id: userId } });
    if (!findUser) throw new HttpException(409, "User doesn't exist");
